 ...
 * @param parentTag
 */
const bucketElements = (elements, parentTag) => {
    let bucket;
    let hasBlock;
    const buckets = [];
    elements.forEach((element, index) => {
        ...
        if (typeof element === 'string') {
            if (parentTag === 'pre') {
                ...
                if (next && exports.hasBlockContent(next)) {
        ...
        const block = exports.hasBlockContent(element);
        if (block !== hasBlock) {
            hasBlock = block;
            bucket = {
                hasBlock,
...
const renderElement = (element, stylesheets, renderers, children, index) => {
    ...
    if (!Element) {
        if (!(element.tag in tags_1.isBlock)) {
            ...
            Element = exports.hasBlockContent(element) ? renderers_1.renderBlock : renderers_1.renderInline;
        }
    }
...
const renderElements = (elements, options, parentTag) => {
    const buckets = exports.bucketElements(elements, parentTag);
    return buckets.map((bucket, bucketIndex) => {
        const rendered = bucket.content.map((element, index) => {
            if (typeof element === 'string') {
                ...
                return exports.renderElement(element, options.stylesheets, options.renderers, undefined, index);
            }
            return exports.renderElement(element, options.stylesheets, options.renderers, exports.renderElements(element.content, element.tag === 'pre' ? Object.assign(Object.assign({}, options), { collapse: false }) : options, element.tag), index);
        });
...
const renderHtml = (text, options) => {
    ...
    const baseStyles = styles_1.createHtmlStylesheet(fontSizeStyle.fontSize, options.resetStyles);
    const parsed = parse_1.default(text);
    const stylesheets = options.stylesheet
        ? Array.isArray(options.stylesheet)
            ? options.stylesheet
            : [options.stylesheet]
        : [];
    const opts = Object.assign(Object.assign({ collapse: true, resetStyles: false }, options), { renderers: Object.assign(Object.assign({}, options.renderers), renderers_1.default), stylesheets: [baseStyles, ...stylesheets, ...parsed.stylesheets] });
    exports.applyStylesheets(opts.stylesheets, parsed.rootElement);
    return (react_1.default.createElement(renderer_1.View, { style: Object.assign(Object.assign({}, styles), fontSizeStyle) }, exports.renderElements(parsed.rootElement.content, opts)));
};
